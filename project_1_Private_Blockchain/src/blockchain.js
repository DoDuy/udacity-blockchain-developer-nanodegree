...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        let self = this;
        return new Promise(async (resolve, reject) => {
            const windowTime = 5*60;
            let messageTime = parseInt(message.split(':')[1]);
            let currentTime = parseInt(new Date().getTime().toString().slice(0,-3));
            if ((currentTime - messageTime) > windowTime){
                reject(new Error("Function submitStar: time elapsed is greater than 5 minutes!"));
            } else {
                var valid = false;
                try {
                    valid = bitcoinMessage.verify(message, address, signature);
                } catch {
                    reject(new Error("Can not run bitcoin verify!"));
                }
                if (!valid) {
                    reject(new Error("Function submitStar: verify message failed!"));
                } else {
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            let block = self.chain.find(bl => bl.hash == hash);
            if(block) {
                resolve(block);
            } else {
                resolve(null);
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            ...
            self.chain.map(block => {
                if (block.height > 0){
                    dataPromises.push(block.getBData());
                }
            })
            Promise.all(dataPromises)
            .then((datas) => {
                datas.forEach(data => {
                    if ("address" in data && data.address === address){
